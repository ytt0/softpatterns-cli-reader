...
namespace SoftPatterns.Cli.Reader.BuildTasks
{
    public class VersionTask : Microsoft.Build.Utilities.Task
    {
        ...
        private static readonly Regex DescribeRegex = new("^((?<tag>.*)-g)?(?<sha>[0-9a-f]{40})(?<dirty>-dirty)?$");
        private static readonly Regex DescribeTagRegex = new($"{VersionPattern}-(?<commits>[0-9]+)");
        private static readonly Regex ReleaseBranchVersionRegex = new($"^release/{VersionPattern}$");
